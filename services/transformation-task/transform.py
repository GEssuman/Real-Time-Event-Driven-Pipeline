...
DELTA_ORDERS_PATH = f"{BASE_DELTA_PATH}/orders"
...
def order_level_kpi(orders_df, order_items_df):
    ...
    unprocessed_orders_df.write.format("parquet").mode("overwrite").save(DELTA_ORDERS_PATH)
...
def main():
    ...
    try:
        unprocessed_order_df = spark.read.format("parquet").load(DELTA_ORDERS_PATH)
    except:
