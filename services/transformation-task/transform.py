...
def category_level_kpi(order_items_df, products_df):
    try:
        ...
        logging.info("Category-level KPI computation completed.")
        return kpi_df
    except Exception as e:
        logging.error(f"Failed to compute category-level KPIs: {e}")
        return None
...
def order_level_kpi(orders_df, order_items_df):
    try:
        ...
        logging.info("Order-level KPI computation completed.")
        write_to_dynamo(kpi_df.collect(), "order_level")
        return kpi_df, unprocessed_orders_df
    except Exception as e:
        logging.error(f"Failed to compute order-level KPIs: {e}")
        return None, orders_df  # fallback
...
def write_to_dynamo(data, kpi_type):
    ...
    """
    if not data:
        logging.warning(f"No data to write to DynamoDB for {kpi_type}")
        return
    
    try:
        logging.info(f"Writing {kpi_type} KPIs to DynamoDB...")
        ...
        logging.error(f"Error initializing DynamoDB client or table: {outer_e}")
...
def archive_all_csv_files(bucket_name, prefix, archive_prefix="archive/"):
    ...
    """
    logging.info(f"Archiving files in bucket: {bucket_name}, prefix: {prefix}")
    s3 = boto3.client("s3")
...
def main():
    try:
        ...
        if not exists:
            ...
            logging.info("Product data initialized from raw CSV.")
